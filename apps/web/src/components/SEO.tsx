...
const SEO: React.FC<SEOProps> = ({ 
  ...
  const siteName = 'AlternativasBR';
  const fullTitle = title.includes(siteName) ? title : `${title} | ${siteName}`;
  
  // URL absoluta para canonical e images
  const getAbsoluteUrl = (path?: string) => {
    if (!path) return undefined;
    if (path.startsWith('http')) return path;
    
    // Preferência: Variável de ambiente -> Domínio de Produção -> Window Origin (localhost)
    const baseUrl = import.meta.env.VITE_SITE_URL || 'https://alternativasbr.com.br';
    
    // Em desenvolvimento, mantemos o localhost para debug correto das tags
